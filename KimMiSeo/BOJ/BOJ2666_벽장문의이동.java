...
public class BOJ2666_벽장문의이동 {
    ...
    private static void solve(int index, int open1, int open2, int cnt){
        if (index == L){
        ...
        solve(index+1, play[index], open2,  cnt+c);

        c = Math.abs(open2 - play[index]);
        doors[play[index]] = false;
        doors[open2] = true;
        solve(index+1, open1, play[index], cnt+c);
